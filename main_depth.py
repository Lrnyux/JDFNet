...
def get_args():
    ...
    # ==========define the task==============
    parser.add_argument('--task', type=str, default='Depth', choices=['Depth'])
    ...
    # ==========loss function============
    parser.add_argument('--loss_type', type=str, default='singdir', choices=['singdir'])
...
def train_depth():
    args = get_args()
    LOGGER = ConsoleLogger('train_depth_' + args.trial, 'train')
    ...
    for epoch in range(args.max_epoch):
        ...
        for it, batch in enumerate(train_dataloader, 0):
            ...
            loss_term_s1 = Loss_func(imgl, imgr, disp_pred[0], max_disp=args.max_disp, recon_right=0)
            loss_term_s2 = Loss_func(imgl, imgr, disp_pred[1], max_disp=args.max_disp, recon_right=0)
            loss_term_s3 = Loss_func(imgl, imgr, disp_pred[2], max_disp=args.max_disp, recon_right=0)
            loss_s1 = args.w_ssim * loss_term_s1['loss_ssim_left'] + \
                      args.w_l1 * loss_term_s1['loss_l1_left'] + \
                      args.w_grad * loss_term_s1['loss_grad']
            loss_s2 = args.w_ssim * loss_term_s2['loss_ssim_left'] + \
                      args.w_l1 * loss_term_s2['loss_l1_left'] + \
                      args.w_grad * loss_term_s2['loss_grad']
            loss_s3 = args.w_ssim * loss_term_s3['loss_ssim_left'] + \
                      args.w_l1 *  loss_term_s3['loss_l1_left'] + \
            ...
            if it % args.freq_print_train == 0:
                LOGGER.info(
                    'Loss SSIM Left:{:.5f},  Loss L1 Left:{:.5f}, Loss Grad:{:.5f}'.format(
                        loss_term_s3['loss_ssim_left'].item(), loss_term_s3['loss_l1_left'].item(), loss_term_s3['loss_grad'].item()))
        ...
        with torch.no_grad():
            ...
            for it, batch in enumerate(val_dataloader, 0):
                ...
                loss_term = Loss_func(imgl, imgr, disp_pred, max_disp=args.max_disp, recon_right=0)
                loss_val = loss_term['loss_ssim_left']

                loss_val_log.update(loss_val, batch_size)

        message = 'Evaluation=== Loss L1 Stereo {loss.ave:.5f} \t'.format(loss=loss_val_log)
        LOGGER.info(message)
...
def _test_depth(checkpoint=''):
    args = get_args()
    if checkpoint:
        args.load_model = checkpoint
    LOGGER = ConsoleLogger('test_depth_sequence', 'test')
    logdir = LOGGER.getLogFolder()
    LOGGER.info(args)

    test_set = dVRK_dataset_depth(args, stage='Test')
    ...
    with torch.no_grad():
        ...
        for it, batch in enumerate(tqdm(test_dataloader,desc='Processing'), 0):
            ...
            disp_pred_left = disp_pred[:, 0, :, :].view(b, 1, h, w)
            disp_pred_right = -1 * disp_pred[:, 0, :, :].view(b, 1, h, w)

            imgr_recon = apply_disparity(imgl, disp_pred_left, max_disp=args.max_disp)
            imgl_recon = apply_disparity(imgr, disp_pred_right, max_disp=args.max_disp)

            rmse_term = calc_rmse(imgl_recon, imgl)
            psnr_term = calc_psnr(imgl_recon, imgl)
            ssim_term = calc_ssim(imgl_recon, imgl)
