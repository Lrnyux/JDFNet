...
def StereodepthLoss_bidr(left,right,disp_pred,max_disp=96):

    # left [B,C,H,W]
    # disp_pred [B,2,H,W]
    b,c,h,w = left.shape
    disp_left_pred = disp_pred[:,0,:,:].view(b,1,h,w)
    disp_right_pred = disp_pred[:,1,:,:].view(b,1,h,w)

    left_recon = apply_disparity(right,disp_right_pred,max_disp=max_disp)
    right_recon = apply_disparity(left,disp_left_pred,max_disp=max_disp)


    disp_left_smoothness = disp_smoothness(left,disp_left_pred)
    disp_right_smoothness = disp_smoothness(right,disp_right_pred)

    loss_l1_left = torch.mean(torch.abs(left_recon-left))
    loss_l1_right = torch.mean(torch.abs(right_recon-right))

    loss_ssim_left = torch.mean(SSIM_loss(left_recon,left))
    loss_ssim_right = torch.mean(SSIM_loss(right_recon,right))



    loss_grad = torch.mean(torch.abs(disp_left_smoothness)) + torch.mean(torch.abs(disp_right_smoothness))
...
def StereodepthLoss_unidr(left,right,disp_pred,max_disp=96):
    ...
    disp_right_pred = -1 * disp_pred[:,1,:,:].view(b,1,h,w)
    ...
    loss['loss_l1_left'] = loss_l1_left
    loss['loss_ssim_left'] = loss_ssim_left
    # loss['loss_l1_left'] = torch.zeros_like(loss_l1_left)
    # loss['loss_ssim_left'] = torch.zeros_like(loss_ssim_left)
...
def StereodepthLoss_singdr(left,right,disp_pred,max_disp=96,recon_right=0):
...
def FrameFlowLoss_bidr(start,end,flow_pred,max_disp=96):
    ...
    backward_flow = flow_pred[:,2:4,:,:].view(b,2,h,w)

    start_recon = apply_flow(end,backward_flow,max_disp=max_disp)
    end_recon = apply_flow(start,forward_flow,max_disp=max_disp)

    forward_flow_smoothness = flow_smoothness(start,forward_flow)
    backward_flow_smoothness = flow_smoothness(end,backward_flow)

    loss_l1_start = torch.mean(torch.abs(start_recon-start))
    loss_l1_end = torch.mean(torch.abs(end_recon-end))

    loss_ssim_start = torch.mean(SSIM_loss(start_recon,start))
    loss_ssim_end = torch.mean(SSIM_loss(end_recon,end))

    loss_grad = torch.mean(torch.abs(forward_flow_smoothness)) + torch.mean(torch.abs(backward_flow_smoothness))

    loss = {}
    loss['loss_l1_start'] = loss_l1_start
    loss['loss_l1_end'] = loss_l1_end
    loss['loss_ssim_start'] = loss_ssim_start
    loss['loss_ssim_end'] = loss_ssim_end
    loss['loss_grad'] = loss_grad

    return loss

def FrameFlowLoss_singdr(start,end,flow_pred,max_disp=96):
